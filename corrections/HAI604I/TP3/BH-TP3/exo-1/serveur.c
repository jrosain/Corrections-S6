...
{   
    ...
    int ds_client = accept(ds, (struct sockaddr*) &socket_client, &len);
    ...
    char* filepath = malloc(name_size + 17); // ./emission/+nom fichier +\0
    filepath[0] = '\0';
    ...
    printf("Serveur : ouverture du fichier %s, attente de la réception de %li octets\n", filepath, file_size);
    free(filepath);
    ...
    while (total_recv < file_size) {
        ...
        if (res_recv == -1) {
            ...
            printf("Serveur : client déconnecté, impossible de finir le téléversement\n");
            exit(1);
        }
        printf("Serveur : réception de %i octets du client\n", res_recv);
        total_recv += res_recv;
        nb_recv++;
        ...
        while (total_write != res_recv) {
            ...
            printf("Serveur : j'ai écrit un bloc dans le fichier\n"); 
        }
    }

    printf("Serveur : écriture dans le fichier réussie. Vous pouvez vérifier la création du fichier et son contenu.\n");

    // fermeture du fichier
    fclose(file); 
    
    printf("Serveur : j'ai écrit au total : %li octets \n", total_ecrit);
    printf("Serveur : j'ai reçu au total : %li octets \n", total_recv);
    printf("Serveur : j'ai fait un total de %li réceptions \n", nb_recv);
    ...
    if (res_close == -1) {
        perror("Serveur : erreur fermeture de la socket");
        exit(1);
    }
